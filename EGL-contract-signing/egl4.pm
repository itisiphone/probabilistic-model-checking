...
module scheduler
	...
	// A sends bth bit of nth secret, B goes next
	//[receiveB] phase=2 & party=1 & n<N-1 -> (phase'=2) & (party'=2);
	// B sends bth bit of nth secret, A goes next, sending bth bit of n+1th secret
	//[receiveA] phase=2 & party=2 & n<N-1 -> (phase'=2) & (party'=1) & (n'=n+1);
	// A sends bth bit of Nth secret, B goes next
	//[receiveB] phase=2 & party=1 & n=N-1 -> (phase'=2) & (party'=2);
	// B sends bth bit of Nth secret, move to next phase and back to A
	//[receiveA] phase=2 & party=2 & n=N-1 -> (phase'=3) & (party'=1) & (n'=0);	

	// A sends bth bit of n+Nth secret, B goes next
	//[receiveB] phase=3 & party=1 & n<N-1 -> (phase'=3) & (party'=2);
	// B sends bth bit of n+Nth secret, A goes next, sending bth bit of n+1th secret
	//[receiveA] phase=3 & party=2 & n<N-1 -> (phase'=3) & (party'=1) & (n'=n+1);
	// A sends bth bit of 2Nth secret, B goes next
	//[receiveB] phase=3 & party=1 & n=N-1 -> (phase'=3) & (party'=2);
	// B sends bth bit of 2Nth secret, move to next phase and back to A
	//[receiveA] phase=3 & party=2 & n=N-1 & b<L -> (phase'=2) & (party'=1) & (n'=0) & (b'=b+1);	

	// B sends final (Lth) bit of 2Nth secret, move to next phase
	//[receiveA] phase=3 & party=2 & n=N-1 & b=L -> (phase'=4);
	...
	// A sends bth bit of 1st secret to B
	[receiveB] phase=2 & party=1 & n=0 -> (phase'=2) & (party'=2);
	// B sends bth bit of nth secret to A
	[receiveA] phase=2 & party=2 & n<N-1 & N != 1 -> (phase'=2) & (party'=2) & (n'=n+1);
	[receiveA] phase=2 & party=2 & n=N-1 & N != 1 -> (phase'=2) & (party'=1) & (n'=1);
	// A sends bth bit of nth secret to B, starting with second secret
	[receiveB] phase=2 & party=1 & n<N-1 & n > 0 & N != 1-> (phase'=2) & (party'=1) & (n'=n+1);
	// A sends bth bit of Nth secret to B, move to phase 3
	[receiveB] phase=2 & party=1 & n=N-1 & N != 1 -> (phase'=3) & (party'=1) & (n'=0);

	// if N = 1
	[receiveA] phase=2 & party = 2 & N = 1 -> (phase'=3) & (party'=1);
	
	// A sends bth bit of N+1st secret to B
	[receiveB] phase=3 & party=1 & n=0 -> (phase'=3) & (party'=2);
	// B sends bth bit of N+nth secret to A
	[receiveA] phase=3 & party=2 & n<N-1 & N != 1 -> (phase'=3) & (party'=2) & (n'=n+1);
	[receiveA] phase=3 & party=2 & n=N-1 & N != 1 -> (phase'=3) & (party'=1) & (n'=1);
	// A sends bth bit of N+nth secret to B, starting with second secret
	[receiveB] phase=3 & party=1 & n<N-1 & n > 0 & N != 1 -> (phase'=3) & (party'=1) & (n'=n+1);

	// if N = 1
	[receiveA] phase=3 & party = 2 & N = 1 & b<L -> (phase'=2) & (party'=1) & (b'=b+1);
	[receiveA] phase=3 & party = 2 & N = 1 & b=L -> (phase'=4);
	
	// A sends bth bit of 2Nth secret, move to next phase and back to beginning, with b increased
	[receiveB] phase=3 & party=1 & n=N-1 & b<L & N != 1 -> (phase'=2) & (party'=1) & (n'=0) & (b' = b+1);
	// A sends final (Lth) bit of 2Nth secret, move to next phase
	[receiveB] phase=3 & party=1 & n=N-1 & b=L & N != 1 -> (phase'=4);
